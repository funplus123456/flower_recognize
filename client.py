...
class MainWindow(QTabWidget):
    ...
    def initUI(self):
        ...
        btn_change = QPushButton(" 选择测试样本 ")
        btn_change.clicked.connect(self.change_img)
        btn_change.setFont(font)

        img_init = cv2.imread(self.to_predict_name)
        img_init = cv2.resize(img_init, (224, 224))
        cv2.imwrite('images/target1.png', img_init)
        self.img_label.setPixmap(QPixmap('images/xiangrikui.png'))
        ...
        left_layout.addWidget(label_result, 0, Qt.AlignCenter)
        left_layout.addStretch()
        left_layout.addWidget(self.result, 0, Qt.AlignCenter)
        left_layout.addStretch()
    ...
    def change_img(self):
        ...
        if img_name == '':
            ...
            img_init = cv2.resize(img_init, (224, 224))
            cv2.imwrite('images/target1.png', img_init)
            self.img_label.setPixmap(QPixmap('images/target1.png'))

    def predict_img(self):
        img = Image.open('images/target1.png')
        img = np.asarray(img)
